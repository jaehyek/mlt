...
Modify the scripter of Accumulation for Ubuntu version
'''
...
countDB = 0
def getAppAccumulation ( dicttupleappaccum ):
    global countDB
    dblist = glob.glob('*.db')
...
for dictmltlogs in listdictmltlogs:
    ...
    util.CopyMLTLOG( os.path.join(dictmltlogs["log_file_path"] ,  dictmltlogs["log_file_name"]))

    ''' process the gathered log file at current dir '''
    getAppAccumulation(dicttupleAppAccumInfo)
