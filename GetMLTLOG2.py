...
ServiceStatement = "select * from  dmr_service_history  where ( %s )  and  (  cause_cd = 'MDA' or cause_cd = 'MBE'  ) and  receipt_ymd between '%s' and '%s'   "
ServiceStatement = ServiceStatement % ( modelstatementSVC,  startdate, enddate )
print "ServiceStatement = " , ServiceStatement

##LOGstatement = "select * from dmr_basic_info a join  dmr_service_history b on a.esn_imei_no = b.esn_imei_no where ( %s ) and ( %s ) and b.close_dt_rtn_dt >= a.base_ymd and b.cause_cd = 'MDA' and  a.base_ymd between '%s' and '%s' and a.esn_imei_no IS NOT NULL  "
LOGstatement = '''
select  *
from    dmr_basic_info
where   1=1
...
''' ��� MLT LOG List�� ������ �� �Ʒ� statement�� ���. '''
statementMLTLIST = "select * from dmr_basic_info where %s  and base_ymd between '%s' and '%s' "

LOGstatement = LOGstatement % ( modelstatementSVC,modelstatementSVC,  startdate, enddate )

#LOGstatement = statementMLTLIST % ( modelstatementSVC,  startdate, enddate )
...
''' MLT SVC field name '''
SVCfieldnames = ['serial_no','interface_ymd','creation_id','section_cd','receipt_ymd','receipt_ymweek','symptom_cd','defect_type','receipt_no','purc_date','data_type','source_system','creation_dt','cause_cd','esn_imei_no','repair_cd','close_dt_rtn_dt','last_update_dt','symptom_org_cd','corp_cd','last_update_id','model_name','unique_id']
...
fieldnames = ['log_no',  'model_name',   'esn_imei_no',  'serial_no',  'os_version', 'sw_version',  'hw_version',  'base_ymd',  'base_ymweek', 'cust_symptom_cd',   'country_cd',  'log_file_path',  'log_file_name',   'svc_cd',    'rooting_count', 'input_imei_no',  'buyer_cd',   'buyer_model_name',     'update_count',   'phone_type',    'registration',   'process_ymd'  ]
...
for dictmltlogs in listdictmltlogs:
    ...
    dictAdditionalBaicInfo["cust_symptom_cd"] = GetMLTSymptomInfo.mapSymptomCodeToName( dictmltlogs["cust_symptom_cd"])
    dictAdditionalBaicInfo["base_ymd"] = dictmltlogs["base_ymd"]
    dictAdditionalBaicInfo["base_ymweek"] = dictmltlogs["base_ymweek"]

    if boolDBAnalyzePartial == True :
        LogFileCopy = True
        if dictAdditionalBaicInfo["cust_symptom_cd"] == strDBAnalyzePartial :
